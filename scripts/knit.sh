...
### Test file extension
if [[ $extension1 -ne "rmd" || $extension1 -ne "Rmd" ]]; then
...
if [[ $extension2 == "pdf" ]]; then
    echo "library(rmarkdown); rmarkdown::render('"${1}"', 'pdf_document', knit_root_dir='$(pwd)')" >> $tempscript
    Rscript $tempscript || exit 1
fi
if [[ $extension2 == "html" ]]; then
    echo "library(rmarkdown); rmarkdown::render('"${1}"', 'html_document', knit_root_dir='$(pwd)')" >> $tempscript
    Rscript $tempscript || exit 1
fi
